...
# 直接使用 requests 調用 OpenAI API，不使用客戶端庫
def call_openai_api(prompt, model="gpt-4-turbo-preview"):
    api_key = get_api_key()
    if not api_key:
        return "API 密鑰未設置，無法使用 GPT-4 分析功能"
    ...
    try:
        ...
        if response.status_code != 200:
            return f"API 調用失敗: {response.status_code} - {response.text}"
        
        result = response.json()
        return result.get("choices", [{}])[0].get("message", {}).get("content", "無法解析回應")
    
    except Exception as e:
        st.error(f"API 調用出錯: {str(e)}")
        return f"調用過程中發生錯誤: {str(e)}"
...
api_key = get_api_key()
if api_key:
    st.sidebar.success("✅ API 密鑰已設置")
else:
    st.sidebar.warning("⚠️ API 密鑰未設置，GPT-4 分析功能將不可用")
